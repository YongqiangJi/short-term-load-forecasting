...
def build_model(params):
    ...
    model = Sequential()
    model.add(Dense(units=int(params['units1']), activation='relu'))
    model.add(Dense(units=int(params['units2']), activation='relu'))
    #model.add(Dense(units=int(params['units0']), activation='relu'))
    model.add(Dropout(0.1))
    model.add(Dense(48))
    model.build(input_shape=(None,192))
...
# 保存最优模型
test, train_X, train_label, val_X, val_label, pred_test, scaler = get_dataset()
best_model = build_model({'units1': units1, 'units2': units2,  'learning_rate':learning_rate})
best_model.summary()
...
best_history = best_model.fit(train_X, train_label, epochs=epochs, batch_size=batch_size, validation_data=(val_X, val_label),
                ...
                shuffle=True)
#保存最优模型
best_model.save('../2122ukresults/ukbest3layersBPcom3-6.h5')
...
plt.savefig(r'../2122ukresults/ukbest3layersBPcom3-6LOSS.tiff', dpi=300, bbox_inches='tight')
plt.show()
...
np.savetxt('../2122ukresults/ukbest3layersBPcom3-6result.csv', np.round_(P, 2), delimiter=',')
