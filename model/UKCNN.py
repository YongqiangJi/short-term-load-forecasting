...
def get_dataset():
    ...
    # 归一化
    scaler = MinMaxScaler()
    df['residual'] = scaler.fit_transform(df['residual'].values.reshape(-1, 1)).reshape(-1)
...
def build_model(params):
    ...
    model = Sequential()
    model.add(Conv1D(filters=int(params['units1']), kernel_size= 3, activation='relu', input_shape=(train_X.shape[1], train_X.shape[2])))
    ...
    model.add(Conv1D(filters=168, kernel_size= 3, activation='relu'))
...
# 保存最优模型
test, train_X, train_label, val_X, val_label, pred_test, scaler = get_dataset()
best_model = build_model({'units1': units1})
best_model.summary()
...
best_history = best_model.fit(train_X, train_label, epochs=epochs, batch_size=batch_size, validation_data=(val_X, val_label),
                ...
                shuffle=True)
#保存最优模型
best_model.save('../2122ukresults/ukbest3layersCNNresidual.h5')
...
plt.savefig(r'../2122ukresults/ukbest3layersCNNresidualLOSS.tiff', dpi=300, bbox_inches='tight')
plt.show()
...
np.savetxt('../2122ukresults/ukbest3layersCNNresidualresult.csv', np.round_(P, 2), delimiter=',')
