...
best_model.add(GRU(units=144, input_shape=(train_X.shape[1], train_X.shape[2]), return_sequences=True))
#best_model.add(GRU(units=36, return_sequences=True))
#model.add(GRU(units=int(params['units3']), return_sequences=True))
#model.add(GRU(units=int(params['units4']), return_sequences=True))
best_model.add(GRU(units=72, return_sequences=False))
...
best_history = best_model.fit(train_X, train_label, epochs=epochs, batch_size=batch_size, validation_data=(val_X, val_label),
                ...
                shuffle=True)
#保存最优模型
best_model.save('../2122ukresults/fakeukbest2layersGRUcom3-6.h5')
'''
#加载最优模型
...
plt.savefig(r'../2122ukresults/fakeukbest2layersGRUcom3-6LOSS.tiff', dpi=300, bbox_inches='tight')
plt.show()
...
np.savetxt('../2122ukresults/fakeukbest2layersGRUcom3-6result.csv', np.round_(P, 2), delimiter=',')
